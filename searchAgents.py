...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.start, [goal for goal in self.corners])
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                ...
                successors.append(([nextState, goals], action, cost))
        return successors
...
def cornersHeuristic(state, problem):
    ...
    distances = [
        abs(position[0] - corner[0]) + abs(position[1] - corner[1]) for corner in goals
    ]
...
class FoodSearchProblem:
    ...
    def getPacmanPosition(self):
        self.startingGameState.getPacmanPosition()
...
def foodHeuristic(state, problem):
    ...
    distances = [
        mazeDistance(position, corner, problem) for corner in foodGrid.asList()
    ]
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        closest_point = sorted(
            food.asList(),
            # key=lambda x: abs(startPosition[0] - x[0]) + abs(startPosition[1] - x[1]),
            key=lambda x: (
                (startPosition[0] - x[0]) ** 2 + (startPosition[1] - x[1]) ** 2
            )
            ** 0.5,
        )[0]

        x, y = closest_point
        width = problem.food.width
        height = problem.food.height
